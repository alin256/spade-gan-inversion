...
def main():
    ...
    np.savez('true_model',**{'true':O.sim[0][:64,:64,0].T})
    # use a context and render onto a file
    with open(f'TRUE_RUN/{case_name}.DATA','w') as f:
        ctx = Context(f, **{'multnum':O.sim[0][:64,:64,0].T.flatten()})
        tmpl.render_context(ctx)

    # Run file
    com = ['flow','--output-dir=TRUE_RUN','--parsing-strictness=low', f'TRUE_RUN/{case_name}.DATA']
